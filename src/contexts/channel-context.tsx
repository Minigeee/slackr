...
export function ChannelProvider(props: ChannelProviderProps) {
  ...
  const [messages, setMessages] = useState<Omit<FullMessage, 'user'>[]>(
  ...
  const { mutateAsync: toggleReactionMutation } =
    api.message.toggleReaction.useMutation();
    ...
    pusherChannel.bind(
      ...
      (data: { userId: string; name: string }) => {
        ...
        setTypingUsers((prev) => {
          ...
          if (existing) {
            return prev.map((u) =>
              u.id === data.userId ? { ...u, timestamp: Date.now() } : u,
            );
          }
          return [
            ...prev,
            { id: data.userId, name: data.name, timestamp: Date.now() },
          ];
        });
      },
    );
  ...
  const stopTyping = useCallback(() => {
    ...
    pusherClient
      .channel(getChannelName(props.channel.id))
      ?.trigger(EVENTS.TYPING_STOP, {
        userId: user?.id ?? '',
      });
  }, [props.channel.id, user?.id, isTyping]);
